...
describe("SystemPromptAssembler", () => {
  ...
  it("returns override verbatim when override is provided", async () => {
    const result = await assembler.assemble(BASE, [], [], "OVERRIDE");
    expect(result).toBe("OVERRIDE");
  });

  it("override ignores additions", async () => {
  ...
  it("override ignores active tools", async () => {
    const result = await assembler.assemble(
      BASE,
      [],
  ...
  it("returns base when no additions and no tools", async () => {
    const result = await assembler.assemble(BASE, [], []);
    expect(result).toBe(BASE);
  });

  it("does not append empty sections", async () => {
    const result = await assembler.assemble(BASE, [], []);
  ...
  it("appends context addition after base", async () => {
    const result = await assembler.assemble(BASE, [makeAddition("context", "ctx")], []);
    const basePos = result.indexOf(BASE);
    const ctxPos = result.indexOf("ctx");
    expect(ctxPos).toBeGreaterThan(basePos);
  });

  it("appends skills addition after context", async () => {
    const additions = [makeAddition("context", "ctx"), makeAddition("skills", "skills")];
    const result = await assembler.assemble(BASE, additions, []);
    const ctxPos = result.indexOf("ctx");
    const skillsPos = result.indexOf("skills");
    expect(skillsPos).toBeGreaterThan(ctxPos);
  });

  it("appends guidelines addition after skills", async () => {
  ...
  it("appends footer addition after guidelines", async () => {
  ...
  it("correct ordering: base → context → skills → guidelines → footer", async () => {
    const additions = [
  ...
  it("sorts additions within a section by priority ascending", async () => {
  ...
  it("priority sorting is stable across sections", async () => {
  ...
  it("excludes tool without promptSnippet from Available Tools section", async () => {
    const result = await assembler.assemble(BASE, [], [makeTool("no-snippet")]);
    expect(result).not.toContain("## Available Tools");
  });

  it("lists multiple tools with snippets in Available Tools section", async () => {
    const tools = [
      makeTool("r2", { promptSnippet: "R2 storage" }),
      makeTool("d1", { promptSnippet: "D1 database" }),
    ];
    const result = await assembler.assemble(BASE, [], tools);
    expect(result).toContain("- **r2**: R2 storage");
    expect(result).toContain("- **d1**: D1 database");
  });

  it("does not include Available Tools section when all tools lack promptSnippet", async () => {
  ...
  it("does not include Tool Guidelines section when no tool has promptGuidelines", async () => {
    const result = await assembler.assemble(BASE, [], [makeTool("no-guidelines")]);
    expect(result).not.toContain("## Tool Guidelines");
  });

  it("Tool Guidelines section appears after guidelines additions", async () => {
    const tool = makeTool("r2", { promptGuidelines: ["r2 guideline"] });
    const additions = [makeAddition("guidelines", "GUIDELINES")];
    const result = await assembler.assemble(BASE, additions, [tool]);
    expect(result.indexOf("GUIDELINES")).toBeLessThan(result.indexOf("## Tool Guidelines"));
  });

  it("Tool Guidelines section appears before footer", async () => {
    const tool = makeTool("r2", { promptGuidelines: ["r2 guideline"] });
  ...
  it("no trailing double newline when footer is absent", async () => {
    const result = await assembler.assemble(BASE, [makeAddition("context", "CTX")], []);
  ...
  it("assembles all sections together in correct order", async () => {
    const additions = [
