...
All new Durable Objects (`AgentSessionDO`, `TelegramChatDO`) are declared with `new_sqlite_classes` (not the legacy `new_classes`). SQLite-backed DOs are the current Cloudflare default and provide built-in SQL storage.
...
- `src/agent-session-do.ts` — `AgentSessionDO extends DurableObject implements ` (renamed from `do/agent-session.ts`)
- `AgentSessionDO` and `IPiccoloCore` interfaces added to `src/types.ts`
...
| File | Contents |
|---|---|
| `gateways/web/src/types.ts` | `IWebGateway` — browser-facing root interface |
| `gateways/web/src/web-gateway.ts` | `WebGatewayImpl extends RpcTarget` — returns `IUser` stub from `getUser()` |
| `gateways/web/src/index.ts` | Worker fetch handler: `/rpc` → `newWorkersRpcResponse`, static assets |
| `gateways/web/src/worker-configuration.d.ts` | Generated by `wrangler types`; includes `CORE: Fetcher`, `USER_ID` |
| `gateways/web/vitest.config.ts` | `cloudflareTest` with `wrangler.test.jsonc` |
...
| `specs/api.md` | Updated `ISession.prompt()` and `AgentSessionDO.prompt()` with `callback?` parameter; added `ISession.getCallback()` |
...
Gives the LLM the ability to make HTTPS GET/HEAD requests to the public internet, including ranged GET for large files. Validates the tool extension pipeline with zero-binding Workers before introducing storage-bound tools.
...
- Scheme allowlist: only `https://` permitted
- Ranged GET: sends `Range: bytes=N-M` header; throws if server does not respond with 206
- Response truncation at `maxBytes` (hard cap 10 MiB) with notice appended to result text
...
**GET + HEAD only (security):** The original spec supported 6 HTTP methods. Per the security decision made during implementation planning, only `GET` and `HEAD` are supported. POST/PUT/PATCH/DELETE were removed to eliminate request body injection risk and reduce attack surface.

**No custom headers from LLM:** The LLM cannot inject arbitrary request headers. The only header the tool ever sends is `Range`, and only when `byteStart`/`byteEnd` are present. This is explicitly tool-controlled.

**HTTPS only:** `http://` was removed from the scheme allowlist. Only `https://` is permitted.

**Ranged GET — 206 enforcement:** When `byteStart` or `byteEnd` is provided, the tool sends `Range: bytes=N-M` and throws if the server does not respond with `206 Partial Content`. This makes range support failures visible to the LLM immediately, rather than silently returning a full (potentially very large) body.

**`@piccolo/core` path alias:** `tsconfig.json` maps `@piccolo/core` to `../../packages/core/src/types-public.ts` (the types-only export), preventing the fetch-tool from pulling in core's `Env` bindings, which would break tsc.
